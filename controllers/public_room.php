...
class SPODPUBLIC_CTRL_PublicRoom extends OW_ActionController
{
    ...
    public function index(array $params)
    {
        ...
        if ( isset($params['prId']) )
        {
            ...
            for ($i = 0; $i < count(SPODPUBLIC_CTRL_PublicRoom::$nodes); $i++) {
                $normalized_nodes_ids[SPODPUBLIC_CTRL_PublicRoom::$nodes[$i][0]] = $i;
                $original_nodes_ids[$i] = SPODPUBLIC_CTRL_PublicRoom::$nodes[$i][0];
            }
            ...
            $json_graph = '{"nodes": [{"id": ' . $normalized_nodes_ids[SPODPUBLIC_CTRL_PublicRoom::$nodes[0][0]] . ',"name": "' . SPODPUBLIC_CTRL_PublicRoom::$nodes[0][1] . '","fixed": true,"x": 200,"y": 200,"color": "#519c76", "r" : 30},';

            for ($i = 1; $i < count(SPODPUBLIC_CTRL_PublicRoom::$nodes); $i++) {
                $json_graph .= '{"id": ' . $normalized_nodes_ids[SPODPUBLIC_CTRL_PublicRoom::$nodes[$i][0]] . ',"name": "' . str_replace("'", '', SPODPUBLIC_CTRL_PublicRoom::$nodes[$i][1]) . '"';

                $json_graph .= ',"content": "' . str_replace("'", '', SPODPUBLIC_CTRL_PublicRoom::$nodes[$i][2]) . '"';

                switch (SPODPUBLIC_CTRL_PublicRoom::$nodes[$i][3]) {
                    case 0:
                ...
                if ($i == count(SPODPUBLIC_CTRL_PublicRoom::$nodes) - 1) {
                    $json_graph .= '}';
                } else {
                    $json_graph .= '},';
                }
            ...
            for ($i = 0; $i < count(SPODPUBLIC_CTRL_PublicRoom::$links); $i++) {
                $json_graph .= '{"source": ' . $normalized_nodes_ids[SPODPUBLIC_CTRL_PublicRoom::$links[$i][0]] . ',"target": ' . $normalized_nodes_ids[SPODPUBLIC_CTRL_PublicRoom::$links[$i][1]];

                switch (SPODPUBLIC_CTRL_PublicRoom::$links[$i][2]) {
                    case 0:
                        $json_graph .= ',"value": "50"';
                        break;
                    case 1:
                ...
                if ($i == count(SPODPUBLIC_CTRL_PublicRoom::$links) - 1) {
                    $json_graph .= '}';
                } else {
                    $json_graph .= '},';
                }
